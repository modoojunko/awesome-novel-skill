...
# 将 NOVEL_SKILL_HOME 写入 .bashrc（不重复添加）
if ! grep -q "export NOVEL_SKILL_HOME" "$HOME/.bashrc" 2>/dev/null; then
    echo "export NOVEL_SKILL_HOME=\"$DEST\"" >> "$HOME/.bashrc"
    echo "已添加 NOVEL_SKILL_HOME=$DEST 到 ~/.bashrc"
fi
