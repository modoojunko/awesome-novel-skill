...
description: 文风提取。从参考小说中学习写作风格，提取量化规则和定性特征，合并到当前项目的写作配置中。触发："学文风""提取风格""学习写作风格""参考小说""分析文风"。
---
...
从单本参考小说中提取写作风格，三步流程：统计脚本量化分析 → 定性 Agent 分层采样 → 主 Agent 合并注入。产出风格档案 + 更新当前项目的 writing-style.md、anti-ai.md 和提示词。
...
| 项目 story.md 存在？ | 不存在 → **STOP**，请先完成项目初始化 |
...
| 11 | 句式癖好统计 | `structural_tic_usage` — 对 anti-ai.md 8 个模式的全文命中数 |
...
1. `style-profiles/{name}/style-qualitative.md` — 6 维分析结论
...
2. 在章提示词生成时，按叙事段落 function 追加对应类型的参考段落：
...
├── style-metrics.md       # Step 1 脚本产出
├── style-qualitative.md   # Step 2 Agent 产出
└── examples.md              # Step 2 Agent 产出（few-shot 段落池）
```

## 下一步

文风提取完成后，回到正常写作流程。写作风格已按参考小说校准，后续的提示词生成和正文写作将自动应用提取到的规则和范例。
