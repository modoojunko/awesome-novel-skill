...
```
Step 1 脚本必须成功。Step 2 定性分析 Agent 必须产出带原文证据的结论。
Step 3 合并后必须 STOP 等作者确认才能写入文件。
```
...
2. 在 Phase 4 的 per-segment 提示词中，按 segment function 追加对应类型的参考段落：

| Segment function | 注入的 example 类型 |
|-----------------|-------------------|
| `atmosphere` | 开场段落 |
| `dialogue_push` | 对话场景 |
| `action_beat` | 动作场景 |
...
文风提取完成后，回到正常 Phase 流程。写作风格已按参考小说校准，后续的 Phase 4 提示词生成和 Phase 5 正文写作将自动应用提取到的规则和范例。
