---
name: novel-prompt
description: 提示词手动调整参考（已自动执行）。章纲确认后自动生成提示词，此文件提供规则参考供手动调整时查阅。
---

# Novel Prompt — 章提示词生成（手动模式）

> **注意：** 正常流程中提示词已在章纲确认后自动生成（见 `skills/chapter/SKILL.md`），无需手动加载此技能。
> 
> 此技能仅在以下情况使用：
> - 作者说"看一眼提示词"后想手动调整
> - 自动生成的提示词有问题需修复
...
> 摘要：读 chapter.md → 拆叙事段落 → 视角转换 → 双轮净化 → 组装章提示词 → 保存。每步确认。
...
| 检查项 | 操作 |
|--------|------|
| 章纲完整性 | memo（8段）+ emotional_design 全部有值？任一为空 → **STOP**，Read `skills/chapter/SKILL.md` 补全章纲 |
| segments 已存在？ | segments 不持久化到 YAML，每次从 outline + memo + emotional_design 在内存中重新拆分 |
| genre_profile 是否已选择？ | 读取 `settings/writing-style.md` 的 `genre_profile` 字段。为空 → **STOP**，列出 `$NOVEL_SKILL_HOME/references/genre-example/index.md` 下可用类型档案，AskUserQuestion 让作者选择。选择后写入 `genre_profile` 并合并 genre_config 到 writing-style.md，然后继续 |

## Step 0: 叙事功能段落拆分

门禁通过后，Agent 自动将章纲拆分为 5-7 个叙事功能段落（segment），展示给作者确认后写入 chapter.md。
...
1. 读取 chapter.md 的 outline + memo + emotional_design
...
[从 writing-style.md + anti-ai.md 实时提炼 8-15 条核心约束。每条一句话，用大白话。不分类（无🔴🟡🟢），不编号。按致命程度排序——最不能犯的排最前面。]
...
> 以下约束从 `settings/anti-ai.md` 和 `settings/writing-style.md` 实时提炼。
...
读取 `settings/writing-style.md` 的 `genre_profile` 字段。若非空：
- 读 `$NOVEL_SKILL_HOME/references/genre-example/index.md` 的题材列表，查找 genre_profile 对应的 `corpus` 字段
- 读 `$NOVEL_SKILL_HOME/references/genre-example/{corpus}` 取 `role_override` 和 `prompt_segment`
- 检查 `$NOVEL_SKILL_HOME/references/genre-example/{genre_profile}.md` 是否存在：存在则读取，覆盖/追加差异字段（variant 文件已扁平化到根目录）。若有 `power_system_ref`，读对应 corpus 的境界/等级体系描述追加到 prompt_segment 末尾
...
确认后，将 chapter.md status 更新为 `draft`。

## 下一步

完成后引导进入写作流程。当作者说"写正文"时，主 Agent 调用 `agents/pipeline/exec-prose.md` 写作。
