...
> **注意：** 正常流程中 Phase 4 已在章纲确认后自动执行（见 `skills/outline/SKILL.md` 的"自动生成提示词"流程），无需手动加载此技能。
> 
> 此技能仅在以下情况使用：
> - 作者说"看一眼提示词"后想手动调整
> - 自动生成的提示词有问题需修复
> - Phase 4 因中断未自动完成（segments 已填充但提示词不存在）
> 
...
| segments 已存在？ | 若 chapter.yaml 已有 segments（非空列表），跳到展示确认，不重新拆分，等作者确认或调整 |
