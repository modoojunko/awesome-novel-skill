...
1. 读取 `~/.claude/skills/awesome-novel/genre-corpus/` 下的所有 `.yaml` 文件，提取 `id`、`label`、`description`
2. AskUserQuestion 列出可用类型档案 + "暂不选择（手动配置题材）"选项
3. 作者选择后，展示档案摘要：
...
5. 应用类型档案：
   ...
   - 将 `genre_config`（satisfaction_types、chapter_types、pacing_rules、anti_cliches）合并到 `settings/writing-style.yaml` 的 `genre` 字段
   - 将 `genre_config.genre_fatigue_words` 追加到 `settings/anti-ai.yaml` 的 `fatigue_words_zh` 对应分类下
   - `prompt_segment` 保留在档案文件中不动——Phase 4 时读取并注入 per-segment 提示词
