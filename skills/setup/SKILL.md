---
name: novel-setup
description: 小说项目初始化与设定。Phase 1+2。当项目尚未创建、world-setting.md 为空、或用户说"创建项目""讨论设定""设计角色""世界观""写作风格""题材""导入小说"时必须使用。即使只说"帮我开始写小说"也要先加载本技能完成设定。
---
...
**从导入模式进入 Phase 2 时：** Agent 先检查 `settings/writing-style.md` 是否为模板默认内容。若是默认模板 + 反向提取到的风格特征 → 报告差异，让作者选择："保留默认模板 / 用反向提取到的风格覆盖 / 混合调整"。确认后再进入 Phase 2。
...
2. 按 world-setting.md 模板的字段（geography → politics → culture → history → rules → physics → biology → sociology）逐项引导，每项即时确认
3. 全部确认后写入 world-setting.md
4. **诊断：** "这个设定能通过角色经历的场景呈现吗？"

### 角色设定

1. AskUserQuestion 收集角色名列表
2. 按认知6层模型（世界观 → 自我定位 → 价值观 → 能力 → 技能 → 环境）逐角色讨论
3. 填写 故事角色（protagonist / antagonist / supporting / minor）
4. 每角色讨论完创建 `settings/character-setting/[拼音id].md`
...
5. 应用类型档案（合并算法见 `skills/prompt/SKILL.md` Step 2，本处只写结果）：
   ...
   - 合并 genre_config（satisfaction_types / chapter_types / pacing_rules / anti_cliches）到 writing-style.md 的 genre 字段
   - 追加 genre_fatigue_words 到 anti-ai.md 对应分类
...
1. 引导浏览 hooks.md 了解生命周期
2. 有伏笔想法写入，无则留空后续填充


### 生成全局提示词（可选——作者参考文档）

1. 读取 writing-style.md 全部字段，组装 `prompts/global-prompt.md`
2. 用大白话写，不用 emoji 分类（🔴🟡🟢），不编号。内容是写作方法论汇总——角色定位、核心原则、禁止事项、题材规则、技法参考
3. **注意：** 此文件是供作者审阅的参考文档。chapter-loop Step 3 的章提示词直接从 writing-style.md 实时提取约束并用自然语言重述，不从 global-prompt.md 复制粘贴
4. 展示确认。
