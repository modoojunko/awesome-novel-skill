...
4. **诊断：** "这个设定能通过角色经历的场景呈现吗？"（参考 best-practices.md「世界观构建」）
...
5. **逐角色诊断（参考 best-practices.md「人设诊断」）：**
   - "TA 生气时怎么表现？和其他角色有什么不同？"（排查扁平化）
   - "TA 有什么缺点？太完美的人设读者带入不进去"（排查完美人设）
   - "配角除了服务剧情，自己有什么目的？"（排查工具人化）
...
4. **STOP：作者确认或提出调整。** 作者可以：
   ...
   - "用这个但氛围我想改成XX" → 微调后应用
5. 应用类型档案：
   - 写入 `genre_profile: "{id}"` 到 `settings/writing-style.yaml`
   - **读取基类 corpus 文件**（`index.yaml` 中 `corpus` 指向的文件）
   - **检查是否有变体文件**（`variant/{id}.yaml` 存在？）→ 存在则读取，**差异字段覆盖基类的同名字段**（taglines_add/taboos_add 追加到基类列表末尾）
   - **检查变体是否有 `power_system_ref`** → 有则额外读取对应 corpus 文件的力量体系描述（仅提取境界等级信息），追加到 prompt_segment
   - 将合并后的 `genre_config`（satisfaction_types、chapter_types、pacing_rules、anti_cliches）写入 `settings/writing-style.yaml` 的 `genre` 字段
   - 将合并后的 `genre_fatigue_words` 追加到 `settings/anti-ai.yaml` 的 `fatigue_words_zh` 对应分类下
   - 将 `style_blueprint` 和合并后的 `genre_taboos` 留在 corpus 文件中不动——Phase 4 时读取并注入提示词
   - `prompt_segment` 保留在档案文件中不动——Phase 4 时读取并注入
...
3. **注意：** 此文件是供作者审阅的参考文档。Phase 4 的章提示词直接从 writing-style.yaml 实时提取约束并用自然语言重述，不从 global-prompt.md 复制粘贴
4. 展示确认。
5. **STOP：作者确认后告知"设定完成。下一步可以规划章节了。说出'规划章节'开始。"**

## 下一步

完成后引导作者进入 Phase 3。当作者说"规划章节"时，主 Agent Read `skills/outline/SKILL.md` 进入 Phase 3。
