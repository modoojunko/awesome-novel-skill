...
调用 subagent 前，清理主 agent 的读取历史（genre-example、anti-ai 等文件后续不再需要）。减少 subagent 的干扰上下文。
...
## Inputs
- `prompts/vol-{N}-ch-{M}-prompt.md` — 唯一输入（9 层，含元信息/来龙/去脉/角色弧光/场景序列/约束/爽点/文字规则/质感）
...
## 防AI味字面规则（违规即重写）
- 疲劳词：全文"突然"≤3、"然而"≤4、"于是"≤3、"渐渐"≤2
- 句式：连续4句主语不得相同；"不是X而是Y"≤3次/章
- 元叙事：正文禁止出现"本章""下一章""故事还在继续""读者"
- 情绪直述：不用"他感到/她感到/他意识到"开头
- 对话间隔：对话之间必须有动作/环境/内心活动，连续8句纯对话违规

## 防重复规则（违规即重写）
- 段落开头：连续3段不得以相同主语/场景词开头
- 句式交替：禁止连续5句相同句式（陈述/短句/对话交替）
- 描写对象：禁止连续2段描写同一物品/人物的不同属性
- 情绪词：紧张/开心/难过等情绪词单章≤3次
- 意象复用：用过的比喻/意象本章不再用
...
子 agent 执行写完后返回。主 Agent 检查输出文件是否存在，然后保存 AI 原版快照。
...
验证通过后，保存 AI 原版到 `.agent/`：

1. 读取刚生成的草稿 `archives/vol-{N}-ch-{M}-{slug}.draft.md`
2. 创建 `.agent/` 目录（如果不存在）
3. 复制一份到 `.agent/{chapter}-draft-ai.md`

```
# 示例
archives/vol-1-ch-3-scene.draft.md → .agent/vol-1-ch-3-draft-ai.md
```

**注意：** 此快照用于后续归档时的 diff 对比，作家修改前保留原始版本。
