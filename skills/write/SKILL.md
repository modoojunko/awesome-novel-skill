...
> 读 prompt + chapter.md → 调子 agent 写作 → 验证输出。主 Agent 做分发和验证，写作由子 agent 自主执行。
...
1. 确认卷号 `{N}` 和章号 `{M}`
2. 读取 `prompts/vol-{N}-ch-{M}-prompt.md`，确认 6 模块完整
3. 读取 `chapters/vol-{N}-ch-{M}.md` 确认 outline、字数目标、narrative_pov
...
```
## Role
全章正文写作。读提示词文件 + chapter.md 章纲，一次性写完整章正文。

## Scope
- 做：读提示词 + chapter.md + global-prompt.md，按提示词叙事流写整章
- 不做：不修改提示词、不修改章纲、不写其他章、不写 settings/ 下任何文件

## Inputs
- 卷号：{N}
- 章号：{M}
- `chapters/vol-{N}-ch-{M}.md` — 章纲（summary、memo、emotional_design）
- `prompts/vol-{N}-ch-{M}-prompt.md` — 提示词（6 模块）
