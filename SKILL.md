...
## Execution Contract（执行契约 — 优先级最高）

以下规则高于一切流程描述。**任何情况下不得违反。如果你正在考虑跳过某条——停止，你没有选择权。**

| # | 规则 | 说明 |
|---|------|------|
| 1 | **Phase 顺序不可跳** | 禁止跳过任何 Phase。作者说"写第一章"≠跳过设定直接生成。必须从当前 Phase 逐步推进 |
| 2 | **前置检查不可跳** | Phase 3/4 的 author-intent.md + current-focus.md 门禁是强制的——未通过不得进入该 Phase |
| 3 | **STOP 点必须等确认** | 每个 STOP 步骤，必须将结果展示给作者并等待明确确认。禁止"作者没反对就是同意" |
| 4 | **禁止代笔** | 所有 YAML 和正文必须经作者讨论确认后才能写入。禁止参照模板自行填充 |
| 5 | **禁止批量产出** | 每次只推进一个步骤，禁止一次性创建多个 Phase 的文件 |
| 6 | **两轮确认不可合并** | Phase 4 视角转换必须先单独确认，确认后才能生成变体。禁止"一起看效率更高" |

**Agent 常见借口（识别并拒绝）：**
- "作者没回应我就继续往下走" → 必须等待
- "差不多就行，不用那么详细" → 细节决定质量
- "视角转换和变体一起生成效率更高" → 省一步就是省掉作者的控制权
- "模板格式我看懂了，直接帮作者填好" → 禁止代笔
- "作者说写第一章，我直接生成所有文件" → 必须按 Phase 顺序推进
- "章纲的 memo 可以跳过直接转换" → memo 是视角转换的约束来源
- "硬禁止和软建议写一起就行" → 必须🔴🟡🟢三层物理分离
...
| 检查项 | 操作 |
|--------|------|
| 读取 `author-intent.md` | 核心主题已有 / 仍是模板？ |
| 读取 `current-focus.md` | 1-3章聚焦已有 / 仍是模板？ |
| 任一仍是模板 | **STOP** — 先问作者填写，确认后再进入 |
| 两者均已填写 | 向作者报告一致性后进入 |

跳过此门禁 → Phase 4 写作指引缺少作者意图约束，subagent 方向偏离。
...
`author-intent.md`（长周期方向）和 `current-focus.md`（1-3章聚焦）是作者直接表达意图的轻量级文档。Agent 在 Phase 3/4 门禁中必须读取并报告一致性。作者可随时说"更新作者意图"或"更新当前聚焦"来修改。current-focus.md 的更新时机由 Phase 6 步骤 11 的滑动窗口审视触发。
...
跳过此门禁 → 提示词缺少全局方法论或卷上下文，subagent 不知道"怎么写出好小说"或"前面写了什么"。
...
**Subagent 调用模板**（prompt 字段只写不可重复的指令。写作约束全在提示词文件里——不要在此重复）：
...
| **句式单调检测** | 读取 `settings/anti-ai.yaml` 的 sentence_rules：(1) 扫描正文开头 500 字和末尾 500 字，检查是否连续 4 句以上以相同主语或连词开头（他/她/这时/接着/然后/于是/便），超阈值则报告位置和建议替换；(2) 随机抽取 500 字段落，统计短句(<10字)和长句(>50字)数量，不足则标记"句子长度缺乏变化——建议加入短句切断节奏或长句延伸沉浸感" |
| **身体反应模板化检测** | 对照 `settings/anti-ai.yaml` 的 structural_tic_patterns 中"身体部位情绪模板"正则 + sentence_rules 的 max_body_emotion_per_500：统计"眼神/心里/喉咙/手心/指尖/心头"等身体部位情绪表达的总密度。超过 5 次/500 字标记警告——"身体反应表达过于集中，读者会注意到模式"。同时检查是否同一个身体反应在不同章节被不同角色复用（如角色A和角色B都用'心里一沉'表达沮丧） |
...
以上所有借口已在 [Execution Contract](#execution-contract执行契约--优先级最高) 中覆盖。如果你发现自己在用其中任何一个合理化跳步——回去读执行契约。

## Red Flags（诊断清单 — 以下任何现象出现，说明执行契约已被违反）
