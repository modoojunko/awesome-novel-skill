...
| "生成提示词""视角转换" | Phase 3 自动生成 | 路由到 `novel-outline`——提示词在章纲确认后自动组装。作者可直接编辑 prompt 文件手动调整 |
...
| Phase 4 | novel-prompt（已合并到 Phase 3）。章纲确认后自动生成提示词 | 自动执行，见 `skills/outline/SKILL.md` 末尾。手动编辑直接改 prompt 文件 |
