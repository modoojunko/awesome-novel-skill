...
步骤：
1. 读取以下源文件（详见下方映射表）
2. 将上帝视角的章纲转换为沉浸式写作指引（见下方视角转换规则）
3. **将转换后的内容展示给作者审查确认**，作者不满意则调整，直到确认
4. 基于确认后的内容组装提示词 yaml
5. 提供 3 个提示词变体供作者选择（同一章纲内容 + 3 种不同切入角度/叙事策略）
6. 作者不满意则调整
7. 确认后将章节状态从 outline → draft
8. 确保 `prompts/` 目录存在，保存为 `prompts/vol-N-ch-M-prompt.yaml`
