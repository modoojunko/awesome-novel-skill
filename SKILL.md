...
5. **必须先读取 `settings/writing-style.yaml`**，将其中的 `role`、`core_principles`、`possible_mistakes`、`depiction_techniques` 四个字段的内容融合为自然段落（见下方提示词格式）。这四个字段是 subagent 的行为约束，缺失任何一个 subagent 都会放飞。
   - **同时读取 `reader_psychology` 字段**：期待管理、信息落差、情绪节拍融入"写作原则与禁忌"段，让 subagent 知道如何控制读者情绪
   - **同时读取 `desire_engine` 字段**：基础欲望和主动欲望融入"写作要求"段，让 subagent 知道本章要满足读者的什么欲望
   - **同时读取 `immersion_pillars` 字段**：六支柱融入"写作指引"段，作为场景设计的默认模板
   - **同时读取 `emotional_pacing` 字段**：关系递进规则和情绪升级法融入"写作原则与禁忌"段
   - **同时读取 `creative_constitution` 字段**：14 条写作脊梁融入"写作原则与禁忌"段
   - **同时读取 `character_psychology_method` 字段**：六步推导法融入"写作原则与禁忌"段——subagent 写关键场景前必须先推导角色行为
...
2. 文件中已包含场景基调、关键节点、叙述约束、读者情绪设计（本章要让读者感受到什么情绪、章尾要留下什么情绪缺口）和写作要求。写作要求中的字数下限为硬性指标，低于此字数视为不合格。
3. 写作时同步考虑读者的心理状态：
...
4. 在正文前必须先输出 PRE_WRITE_CHECK 自检表（markdown 表格），对齐以下项目：
   | 当前任务 | 本章要完成的动作 |
   | 读者在等什么 | 兑现/延迟/制造新缺口，具体处理方式 |
   | 该兑现的 / 暂不掀的 | 本章确认处理的伏笔清单 |
   | 章尾必须发生的改变 | 1-3 条（信息/关系/物理/权力） |
   | 不要做 | 本章红线 |
   每项都必须写清楚本章的具体处理方式，不得留空或写"无"。写完自检表后开始正文。
5. 禁止以下句式和行为：
...
subagent 返回的正文在展示给作者之前，主 Agent 必须先检查以下九项（含 AI 味检测、情绪缺口检测、跨章情绪单调检测）。**任意一项触发，不展示正文，向作者报告具体问题，由作者决定下一步：重新调用 subagent、主 Agent 按指令重写、或作者给出方向后主 Agent 修改。**

检测时参考 `settings/anti-ai.yaml` 中的 fatigue_words、sentence_rules、paragraph_rules、dialogue_rules、emotional_cadence 五个规则组。
...
7. 运行钩子健康检查（参考 hooks.yaml 的 hook_health_rules）：
   ...
   - 将检查结果报告给作者
8. 将 chapter.yaml 的 status 更新为 `archived`
9. 更新 story.yaml 的 chapters 列表中对应条目
