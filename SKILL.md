...
| chapter.yaml status = `outline`，memo 完整，segments 为空，提示词文件不存在 | Phase 3 刚完成——章纲确认后自动执行 Phase 4 | → `novel-outline`（章纲确认后自动拆 segment → 生成提示词 → 更新为 draft） |
| chapter.yaml status = `outline`，segments 已填充，提示词不存在 | Phase 4 修复/手动模式——需手动补提示词 | → `novel-prompt` |
...
- 目标 Phase 4 → 已移入 Phase 3 自动执行。手动进入 Phase 4（`novel-prompt`）时检查 segments 是否已拆分
