...
6. 作者满意后，将正文写入 `chapters/vol-N-ch-M.yaml` 的 content 字段（draft 落盘）
7. 进入 Phase 6 归档流程
...
Agent(
  ...
  prompt: "读取 prompts/vol-{N}-ch-{M}-prompt.yaml，严格按照其中的 role、core_principles、context、content 字段写作。content 中包含写作要求（字数、节奏、结尾方向），必须遵守。一次性生成完整章节正文。只返回正文内容。"
)
```
...
步骤：
1. 从 `chapters/vol-N-ch-M.yaml` 的 content 字段读取正文，写入 `archives/vol-{N}-ch-{M}-{slugified-title}.md`
...
- [ ] 正文已写入 chapters/vol-N-ch-M.yaml 的 content 字段后再进入归档
