...
5. **必须先读取 `settings/writing-style.yaml`**，将其中的 `role`、`core_principles`、`possible_mistakes`、`depiction_techniques` 四个字段原样复制到提示词 yaml 中。这四个字段是 subagent 的行为约束，缺失任何一个 subagent 都会放飞。
6. 读取其余源文件，按映射表组装提示词 yaml 骨架
7. 将第一轮确认后的沉浸式内容填入提示词的 `content` 字段
8. 基于同一 content，生成 3 个提示词变体（不同切入角度/叙事策略）
9. **STOP：将 3 个变体展示给作者选择，作者不满意则调整，直到确认。**
10. 作者确认后，将章节状态从 outline → draft
11. 确保 `prompts/` 目录存在，保存为 `prompts/vol-N-ch-M-prompt.yaml`
...
| chapter.yaml > outline | `content` | 章纲经视角转换后填入（见下方规则） |
...
2. subagent 的 prompt 必须包含以下硬性约束（见下方完整示例）
...
Agent(
  ...
  prompt: "读取 prompts/vol-{N}-ch-{M}-prompt.yaml。

严格按以下要求写作：

1. 遵守 prompt yaml 中 role、core_principles、possible_mistakes、depiction_techniques 的所有约束。
2. 遵守 content 字段中的场景基调、关键节点、叙述约束和写作要求。写作要求中的字数下限为硬性指标，低于此字数视为不合格。
3. 禁止以下句式和行为：
...
subagent 返回的正文在展示给作者之前，主 Agent 必须先检查以下四项。**任意一项触发，不展示正文，向作者报告具体问题，由作者决定下一步：重新调用 subagent、主 Agent 按指令重写、或作者给出方向后主 Agent 修改。**

| 触发条件 | 报告内容 |
|---------|---------|
| 字数不达标 | 告知作者：实际字数 vs 要求字数下限，询问是否接受当前长度还是重写 |
| 上帝视角摘要 | 摘出正文中典型的概述句式（如"他经历了""本章讲述了"），向作者说明为什么这些段落读起来像梗概而非叙事，询问是否按提示词要求重写 |
| 违反核心约束 | 指出正文中违反的具体约束条款（引用 prompt yaml 中的 prohibit 条目），说明违规位置 |
...
content: |
  [经视角转换后的沉浸式写作指引，包含：场景基调、关键节点、叙述约束、写作要求（字数/节奏/结尾）]
```
...
- 提示词 yaml 缺失 writing-style 四个字段（role / core_principles / possible_mistakes / depiction_techniques）中的任何一个——缺少约束 subagent 必然放飞
...
- [ ] 提示词 yaml 包含完整上下文，content 字段为沉浸式写作指引（非上帝视角摘要）
- [ ] 提示词 yaml 必须包含 writing-style 四个字段：role / core_principles / possible_mistakes / depiction_techniques
