...
5. 读取以下源文件，按映射表组装提示词 yaml 骨架
6. 将第一轮确认后的沉浸式内容填入提示词的 `content` 字段
7. 基于同一 content，生成 3 个提示词变体（不同切入角度/叙事策略）
8. **STOP：将 3 个变体展示给作者选择，作者不满意则调整，直到确认。**
9. 作者确认后，将章节状态从 outline → draft
10. 确保 `prompts/` 目录存在，保存为 `prompts/vol-N-ch-M-prompt.yaml`
...
**视角转换规则**（章纲 → 提示词 content 字段）：

章纲是给作者看的大纲，上帝视角概述剧情。提示词的 content 是给 subagent 看的写作指引，必须去掉上帝视角，否则 subagent 会写成剧情摘要而非沉浸式叙事。
