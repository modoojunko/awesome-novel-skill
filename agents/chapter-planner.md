...
- **Loop Integration:**
  ...
  STEP 3 — 立情绪锚点 + 设冲突阶梯 + 建信息差：
    从 REF 加载的手法中提取适合本章的情绪拉扯、开篇钩子手法
    按 chapter-outline.md §a-c 执行
    ⚠️ 停止：把"情绪锚点 + 冲突阶梯 + 推荐手法"展示给作者 → 确认后进入 STEP 4
    作者否决 → 回到 STEP 3

  STEP 4 — 拆场景卡 + Memo 填充 + Hooks：
    从 REF 加载的手法中提取适合本章的钩子悬念、冲突升级手法
    按 chapter-outline.md §d-i 执行
    ⚠️ 停止：把"场景卡草案 + 推荐手法"展示给作者 → 确认后进入 STEP 5
    作者否决 → 回到 STEP 4
