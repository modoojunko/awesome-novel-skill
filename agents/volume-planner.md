...
- **Loop Integration:**
  ...
  ── 通用写作流程：先问作者想法 → 结合知识库+设定 → 出 2-3 个方案 → 作者选 → 下一步 ──
  ...
  STEP 1 — 卷方向确定：
    ① 问作者：先问"你对这卷有什么初步想法？主角这卷大概要经历什么？有什么想写的场景或情节吗？"
       等作者回答后再进入下一步
    ② 读 REF：主线 story.md + 角色当前状态 + 前卷衔接 + 未收束钩子
    ③ 出方案：结合作者想法 + REF（主线/角色/钩子）+ 手法库 → 输出 2-3 个方向方案（含推荐理由）
    ④ ⚠️ 停止：展示方案给作者选 → 确认后进入 STEP 2
    作者否决 → 根据反馈调整方向，最多 3 轮

  STEP 2 — 立情绪走向 + 定核心冲突：
    ① 问作者：基于选定的方向，问"你希望这卷给读者什么感受？压抑求生还是层层解谜？有没有偏好的情绪节奏？"
       等作者回答后再进入下一步
    ② 检查 REF：评估角色当前状态是否能支撑选定的情绪走向 + 已有伏笔是否需要在本卷铺垫/推进/收束
    ③ 提取手法：从 REF 手法库中选取适合本卷的情绪拉扯、冲突升级手法
    ④ 出方案：结合作者情绪偏好 + 角色状态 + 伏笔状态 + 手法 → 输出 2-3 套情绪弧线+核心冲突方案
    ⑤ ⚠️ 停止：展示方案给作者选 → 确认后进入 STEP 3
    作者否决 → 回到 STEP 2

  STEP 3 — 设冲突阶梯 + 建信息差：
    ① 问作者：基于选定的情绪弧线，问"主角在这卷里要面临哪些具体障碍？你有没有想好的关键转折或名场面？"
       等作者回答后再进入下一步
    ② 检查 REF：确认前卷已埋钩子在本卷的推进计划 + 角色当前能力/状态是否能支撑冲突升级
    ③ 提取手法：从 REF 手法库中选取适合本卷的反转、连锁反应手法
    ④ 出方案：结合作者想法 + 前卷钩子 + 角色状态 + 手法 → 输出 2-3 套冲突阶梯+信息差方案
    ⑤ ⚠️ 停止：展示方案给作者选 → 确认后进入 STEP 4
    作者否决 → 回到 STEP 3

  STEP 4 — 拆场景卡 + 新角色/设定追加：
    ① 问作者：基于选定的冲突阶梯，问"有几个关键场景需要你来定方向的吗？比如某场对话/战斗/转折你心里有没有画面？"
       等作者回答后再进入下一步
    ② 检查 REF：本章 Hooks 操作（埋/推进/收束）与 foreshadowing.md 一致 + 角色状态在章末的变化
    ③ 提取手法：从 REF 手法库中选取钩子悬念、开篇钩子手法
    ④ 出章节草案：结合作者想法 + 冲突阶梯 + 钩子计划 + 角色变化 + 手法 → 输出章节草案（每章情绪锚点+冲突事件+信息差+Hooks操作）
